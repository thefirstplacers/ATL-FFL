...
export default function TransactionsList({ transactions }: { transactions: ProcessedTransaction[] }) {
  ...
  return (
    <div>
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filter === f.key ? 'bg-gold text-navy' : 'bg-surface text-text-secondary hover:bg-surface-hover'
            }`}
          >
            {f.label}
          </button>
      ...
      <div className="text-text-muted text-sm mb-4">{filtered.length} transactions</div>

      {/* Transaction List */}
      <div className="space-y-3">
        {paginated.map((txn) => {
          const typeInfo = typeLabels[txn.type] || { label: txn.type, color: 'bg-surface text-text-secondary', icon: '📄' };
          return (
            <div key={txn.id} className="glass-card p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                {txn.waiverBid > 0 && (
                  <span className="text-gold text-xs font-bold">${txn.waiverBid} FAAB</span>
                )}
              </div>
